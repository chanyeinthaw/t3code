...
import { EnvironmentAuthInvalidError } from "@pulse/contracts";
import {
...
describe("remote environment authorization", () => {
  ...
  it.effect("loads remote session state and websocket tickets over bearer auth", () =>
    Effect.gen(function* () {
      ...
      expectFetchCall(fetch.calls, 1, {
        url: "https://remote.example.com/.well-known/pulse/environment",
        method: "GET",
      });
      expectFetchCall(fetch.calls, 2, {
  ...
  it.effect("fails hung fetch requests on the configured timeout", () =>
    Effect.gen(function* () {
      ...
      expect(error.message).toBe(
        "Remote environment endpoint http://remote.example.com/.well-known/pulse/environment timed out after 25ms.",
      );
