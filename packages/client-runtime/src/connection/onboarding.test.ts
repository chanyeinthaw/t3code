import { AuthStandardClientScopes, EnvironmentId } from "@pulse/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/pulse/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/pulse/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("Pulse Test");
    }),
  );
