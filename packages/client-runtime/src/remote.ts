...
import { encodeOAuthScope } from "@pulse/shared/oauthScope";
import { httpHeaderRedactionLayer } from "@pulse/shared/httpObservability";
...
export const fetchRemoteEnvironmentDescriptor = Effect.fn(
  ...
  const client = yield* makeEnvironmentHttpApiClient(input.httpBaseUrl);
  return yield* executeRemoteRequest(
    remoteEndpointUrl(input.httpBaseUrl, "/.well-known/pulse/environment"),
    input.timeoutMs ?? DEFAULT_REMOTE_REQUEST_TIMEOUT_MS,
