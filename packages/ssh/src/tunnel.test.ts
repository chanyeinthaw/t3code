...
describe("ssh tunnel scripts", () => {
  it("builds the remote pulse runner with npx and npm fallbacks", () => {
    const script = buildRemoteT3RunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "T3_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec pulse "$@"');
    assert.include(script, "exec npx --yes 'pulse@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'pulse@latest' -- \"$@\"");
    assert.include(script, "could not install 'pulse@latest'");
  ...
  it("shell-quotes package specs in the remote pulse runner", () => {
    const script = buildRemoteT3RunnerScript({
      packageSpec: "t3@nightly; touch /tmp/pulse-owned",
    });

    assert.include(script, "exec npx --yes 't3@nightly; touch /tmp/pulse-owned' \"$@\"");
    assert.include(script, "exec npm exec --yes 't3@nightly; touch /tmp/pulse-owned' -- \"$@\"");
    assert.notInclude(script, "exec npx --yes t3@nightly; touch /tmp/pulse-owned");
  });

  it("builds the remote pulse runner with a node script override", () => {
    const script = buildRemoteT3RunnerScript({
  ...
  it("uses the remote pulse runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Pulse server did not become ready");
