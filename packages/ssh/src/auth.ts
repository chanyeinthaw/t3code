...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@pulse/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "pulse-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Pulse re-runs ssh with a cached password
...
printf 'Pulse ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Pulse re-runs\r
...
[Console]::Error.WriteLine("Pulse ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "pulse-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "pulse" }),
  };
});
