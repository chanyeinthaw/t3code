...
export const CLOUDFLARED_PATH_ENV_NAME = "PULSE_CLOUDFLARED_PATH";
  ...
  const installUnlocked = Effect.fn("cloudflared.installUnlocked")(function* (
    ...
    if (!releaseAsset) {
      return yield* new RelayClientInstallError({
        ...
        message: `Pulse does not provide a managed relay client binary for ${platform}-${arch}.`,
      });
    }
