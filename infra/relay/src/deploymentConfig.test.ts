...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("pulse-relay-traces", "prod")).toBe("pulse-relay-traces-prod");
    expect(relayResourceNameForStage("pulse-relay-traces", "dev_julius")).toBe(
      "pulse-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "pulserelay.com", hash)).toBe(
      "prod-abcdef0123456789.pulserelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "pulserelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
