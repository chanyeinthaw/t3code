...
export const RelayObservability = Effect.gen(function* () {
  ...
  const traces = yield* Axiom.Dataset("RelayTracesDataset", {
    name: relayResourceNameForStage("pulse-relay-traces", stage),
    kind: "otel:traces:v1",
    description: "Pulse relay Worker HTTP request spans.",
    retentionDays: 30,
    useRetentionPeriod: true,
  });

  const workerIngestToken = yield* Axiom.ApiToken("RelayWorkerAxiomIngestToken", {
    name: relayResourceNameForStage("pulse-relay-otel-ingest", stage),
  ...
  const mobileIngestToken = yield* Axiom.ApiToken("RelayMobileAxiomIngestToken", {
    name: relayResourceNameForStage("pulse-mobile-otel-ingest", stage),
    description: "Owned by Alchemy. Scoped OTLP ingest token for Pulse mobile spans.",
    datasetCapabilities: Output.map(traces.name, (dataset) => ({
      [dataset]: { ingest: ["create" as const] },
    })),
  });
  ...
  const clientIngestToken = yield* Axiom.ApiToken("RelayClientAxiomIngestToken", {
    name: relayResourceNameForStage("pulse-relay-client-otel-ingest", stage),
  ...
  yield* Axiom.View("RelayRecentSpansView", {
    name: relayResourceNameForStage("pulse-relay-recent-spans", stage),
...
export const makeRelayTraceLayer = (input: {
  ...
  Layer.effect(
    ...
    OtlpTracer.make({
      ...
      resource: {
        serviceName: "pulse-relay-worker",
        attributes: {
