...
> [!WARNING]
> Pulse Connect is currently in private beta. Join the waitlist in the app under Settings > Pulse Connect.
...
The relay is intentionally not in the hot path for normal Pulse traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [Pulse Connect architecture overview](../../docs/cloud/pulse-connect-auth-flow.html) for the larger system
design.
...
- Linking Pulse environments to a cloud account.
...
```sh
vp run --filter pulse-relay deploy
```
...
```sh
vp run --filter pulse-relay deploy -- --stage prod
vp run --filter pulse-relay deploy -- --env-file .env.local
```
...
- [Pulse Connect Clerk Setup](../../docs/cloud/pulse-connect-clerk.md) for Clerk keys, JWT templates, and waitlist
  setup.
- [Relay Observability](../../docs/relay-observability.md) for deployment tracing and diagnostics.
- [Pulse Connect Architecture Overview](../../docs/cloud/pulse-connect-auth-flow.html) for the full link,
