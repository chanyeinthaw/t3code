...
const remoteDebuggingPort = process.env.PULSE_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line pulse/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.PULSE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.PULSE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  spawnSync("pkill", ["-f", "--", `--pulse-dev-root=${desktopDir}`], { stdio: "ignore" });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--pulse-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
