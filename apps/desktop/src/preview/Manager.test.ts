...
const browserSessionLayer = Layer.succeed(
  ...
  BrowserSession.BrowserSession.of({
    getPartition: () => Effect.succeed("persist:pulse-preview-test"),
    isPartition: (partition) => partition.startsWith("persist:pulse-preview-"),
...
const environmentLayer = Layer.succeed(
  ...
  DesktopEnvironment.DesktopEnvironment.of({
    browserArtifactsDir: "/tmp/pulse/dev/browser-artifacts",
...
describe("PreviewManager", () => {
  ...
  effectIt.effect("captures a PNG screenshot into browser artifacts", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        expect(mkdir).toHaveBeenCalledWith("/tmp/pulse/dev/browser-artifacts");
  ...
  effectIt.effect("reveals only files inside the configured browser artifact directory", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        yield* manager.revealArtifact("/tmp/pulse/dev/browser-artifacts/browser-screenshot-test.png");

        expect(showItemInFolder).toHaveBeenCalledWith(
          "/tmp/pulse/dev/browser-artifacts/browser-screenshot-test.png",
        );
        const exit = yield* Effect.exit(manager.revealArtifact("/tmp/pulse/dev/settings.json"));
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
        expect(error).toMatchObject({
          _tag: "PreviewArtifactPathOutsideDirectoryError",
          artifactPath: "/tmp/pulse/dev/settings.json",
          artifactDirectory: "/tmp/pulse/dev/browser-artifacts",
        });
  ...
  effectIt.effect("copies screenshot artifacts to the system clipboard", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        const artifactPath = "/tmp/pulse/dev/browser-artifacts/browser-screenshot-test.png";
        ...
        const exit = yield* Effect.exit(
          manager.copyArtifactToClipboard("/tmp/pulse/dev/settings.json"),
        );
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
