...
const OVERLAY_ATTRIBUTE = "data-pulse-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--pulse-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--pulse-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    ...
    "--pulse-background": theme.background,
    "--pulse-foreground": theme.foreground,
    "--pulse-popover": theme.popover,
    "--pulse-popover-foreground": theme.popoverForeground,
    "--pulse-primary": theme.primary,
    "--pulse-primary-foreground": theme.primaryForeground,
    "--pulse-muted": theme.muted,
    "--pulse-muted-foreground": theme.mutedForeground,
    "--pulse-accent": theme.accent,
    "--pulse-accent-foreground": theme.accentForeground,
    "--pulse-border": theme.border,
    "--pulse-input": theme.input,
    "--pulse-ring": theme.ring,
    "--pulse-font-sans": theme.fontSans,
    "--pulse-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-pulse-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--pulse-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-pulse-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
