    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          PULSE_PORT: "9999",
          PULSE_MODE: "desktop",
          PULSE_DESKTOP_LAN_HOST: "192.168.1.50",
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "pulse-desktop-backend-config-test-",
    });
...
describe("DesktopBackendConfiguration", () => {
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.PULSE_PORT);
        assert.isUndefined(first.env.PULSE_MODE);
        assert.isUndefined(first.env.PULSE_DESKTOP_LAN_HOST);
        ...
        assert.equal(first.bootstrap.pulseHome, environment.baseDir);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const linuxEntryPath = "/tmp/pulse code's launch/entry file.mjs";
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.equal(config.bootstrap.pulseHome, environment.baseDir);
          assert.isTrue(Option.isNone(config.preflightFailure));
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
  ...
  it("resolvePrimaryLabel is runSync-safe against the real WSL availability probe", async () => {
    ...
    // oxlint-disable-next-line pulse/no-manual-effect-runtime-in-tests -- This test intentionally replicates the sync IPC handler's runSync path to catch a regression to async-only resolution; it.effect would mask it.
    const runtime = ManagedRuntime.make(
      DesktopBackendConfiguration.layer.pipe(
        ...
        Layer.provideMerge(makeEnvironmentLayer("/tmp/pulse-wsl-isavailable", { platform: "win32" })),
    ...
    try {
      ...
      // oxlint-disable-next-line pulse/no-manual-effect-runtime-in-tests -- Same reason: this is the synchronous resolution the IPC handler performs.
