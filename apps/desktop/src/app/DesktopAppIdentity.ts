...
const AppPackageMetadata = Schema.Struct({
  pulseCommitHash: Schema.optional(Schema.String),
});
...
>()("@pulse/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.pulseCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
