...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/Pulse.app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/Pulse.app/Contents/Resources",
      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(input.legacyPathExists === true && path.includes("Pulse (Alpha)")),
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"pulseCommitHash":"abcdef1234567890"}'),
          }),
        ),
  ...
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Pulse (Alpha)");
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Pulse"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Pulse");
