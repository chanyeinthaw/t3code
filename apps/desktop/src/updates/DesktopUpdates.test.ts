...
import type { DesktopUpdateState } from "@pulse/contracts";
...
function makeHarness(options: UpdatesHarnessOptions = {}) {
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    homeDirectory: `/tmp/pulse-desktop-updates-home-${process.pid}`,
    platform: "darwin",
    processArch: "x64",
    appVersion: "1.2.3",
    appPath: "/repo",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          PULSE_HOME: `/tmp/pulse-desktop-updates-test-${process.pid}`,
          PULSE_DESKTOP_MOCK_UPDATES: "true",
          PULSE_DESKTOP_MOCK_UPDATE_SERVER_PORT: "4141",
          ...options.env,
        }),
      ),
    ),
