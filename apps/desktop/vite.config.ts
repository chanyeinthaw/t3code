  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["@sats-lab/pulse#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env PULSE_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["@sats-lab/pulse#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@pulse/"),
      },
