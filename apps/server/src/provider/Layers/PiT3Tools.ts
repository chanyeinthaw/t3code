...
const T3_CAPABILITY_TOOL_NAME = "t3_capability";
const T3_EXECUTE_TOOL_NAME = "t3_execute";

export const T3_PI_TOOL_NAMES = [T3_CAPABILITY_TOOL_NAME, T3_EXECUTE_TOOL_NAME] as const;

const T3_BROWSER_TOOL_GUIDELINES = [
  "Use t3_capability to discover T3 Code-native capabilities before using external browser automation.",
  "For browser work in T3 Code, prefer t3_execute preview operations: first run preview.status, call preview.open if no automation-capable preview is attached, then use preview.snapshot before preview.click/type/press/scroll/evaluate/waitFor.",
  "Do not switch to standalone Playwright, Chrome, global browser skills, or agent-browser unless t3_execute reports the T3 preview is unsupported/unavailable or the user explicitly asks for another browser.",
];
...
const T3ExecuteParameters = {
  ...
  properties: {
    operation: {
      ...
      description:
        "T3 operation to execute. Use t3_capability for operation details and expected input.",
    },
    input: {
      ...JsonObjectSchema,
...
type T3CapabilityParams = {
  readonly query?: string;
  readonly capability?: string;
};

type T3ExecuteParams = {
  readonly operation: string;
  readonly input?: unknown;
};

interface T3CapabilityOperationDescription {
  readonly operation: string;
...
const PREVIEW_OPERATIONS: ReadonlyArray<T3CapabilityOperationDescription> = [
  {
  ...
  return {
    capabilities: includePreview
      ? [
          {
            id: "preview",
            title: "T3 Code collaborative browser",
            summary:
              "Control the user-visible desktop preview browser scoped to this T3 Code thread.",
            recommendedWorkflow: [
              "Call t3_execute with operation='preview.status' first.",
...
export function makePiT3Tools(input: MakePiT3ToolsInput): ToolDefinition[] {
  ...
  const capabilityTool: ToolDefinition = {
    name: T3_CAPABILITY_TOOL_NAME,
    label: "T3 Capability",
    description: "Search T3 Code-native capabilities available to this Pi session.",
    promptSnippet: "Search T3 Code-native capabilities such as the collaborative browser preview",
    promptGuidelines: T3_BROWSER_TOOL_GUIDELINES,
    parameters: T3CapabilityParameters as never,
    async execute(_toolCallId, params) {
      const typedParams = params as T3CapabilityParams;
  ...
  const executeTool: ToolDefinition = {
    name: T3_EXECUTE_TOOL_NAME,
    label: "T3 Execute",
    description: "Execute an allowlisted T3 Code-native operation discovered via t3_capability.",
    promptSnippet: "Execute T3 Code-native operations discovered via t3_capability",
    promptGuidelines: T3_BROWSER_TOOL_GUIDELINES,
    parameters: T3ExecuteParameters as never,
    async execute(_toolCallId, params) {
      const typedParams = params as T3ExecuteParams;
      ...
      if (!operation) {
        return {
          content: [
            {
              type: "text",
              text: `Unknown T3 operation '${typedParams.operation}'. Call t3_capability to list supported operations.`,
            },
          ],
      ...
      if (!broker) {
        return {
          content: [
            {
              type: "text",
              text: "T3 preview automation is unavailable because no active browser broker is running.",
            },
          ],
      ...
      try {
        ...
        const message = isOwnerMissing
          ? `${baseMessage} Make sure the T3 Code desktop preview panel is open for this thread and that you are running the T3 Code desktop app (browser-only builds cannot host the preview automation bridge).`
          : baseMessage;
        return {
          content: [
            {
              type: "text",
              text: `T3 execute failed for '${typedParams.operation}': ${message}`,
            },
          ],
          details: {
            operation: typedParams.operation,
