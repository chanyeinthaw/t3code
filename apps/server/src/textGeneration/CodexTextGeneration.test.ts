...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@pulse/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "pulse-codex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__PULSE_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__PULSE_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "pulse-codex-text-" });
