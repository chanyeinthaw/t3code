import { ClaudeSettings, ProviderInstanceId } from "@pulse/contracts";
...
const ClaudeTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "pulse-claude-text-generation-test-",
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "pulse-claude-text-" });
