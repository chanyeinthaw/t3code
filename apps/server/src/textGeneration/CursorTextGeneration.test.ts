...
import { CursorSettings, ProviderInstanceId } from "@pulse/contracts";
...
const CursorTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "pulse-cursor-text-generation-test-",
...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.gen(function* () {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "pulse-cursor-text-acp-"));
...
it.layer(CursorTextGenerationTestLayer)("CursorTextGeneration", (it) => {
  it.effect("uses ACP model config options instead of raw CLI model ids", () => {
    const requestLogDir = mkdtempSync(path.join(os.tmpdir(), "pulse-cursor-text-log-"));
  ...
  it.effect("closes the ACP child process after text generation completes", () => {
    const exitLogDir = mkdtempSync(path.join(os.tmpdir(), "pulse-cursor-text-exit-log-"));
