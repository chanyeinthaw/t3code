...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "pulse-terminal-" });
...
it.layer(
  ...
  it.effect("attaches to exited sessions without restarting them", () =>
    Effect.gen(function* () {
      ...
      const unsubscribe = yield* manager.attachStream(
        openInput({
          env: {
            PULSE_WORKTREE_PATH: "/tmp/should-not-restart",
          },
      ...
      const unsubscribe = yield* manager.attachStream(
        {
          ...openInput({
            env: {
              PULSE_WORKTREE_PATH: "/tmp/restart-requested",
            },
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      expect(spawnInput.env.PULSE_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            PULSE_PROJECT_ROOT: "/repo",
            PULSE_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.PULSE_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.PULSE_WORKTREE_PATH, "/repo/worktree-a");
