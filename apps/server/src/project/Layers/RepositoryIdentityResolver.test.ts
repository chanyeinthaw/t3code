...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pulse-repository-identity-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:T3Tools/pulse.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/t3tools/pulse");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("t3tools/pulse");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("t3tools");
      expect(identity?.name).toBe("pulse");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      const repoRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pulse-repository-identity-nested-root-test-",
      });
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:T3Tools/pulse.git"]);
  ...
  it.effect("returns null for non-git folders and repos without remotes", () =>
    Effect.gen(function* () {
      ...
      const nonGitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pulse-repository-identity-non-git-",
      });
      const gitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pulse-repository-identity-no-remote-",
      });
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pulse-repository-identity-upstream-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/pulse.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:T3Tools/pulse.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pulse-repository-identity-nested-group-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:T3Tools/platform/pulse.git"]);

      const resolver = yield* RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/t3tools/platform/pulse");
      expect(identity?.displayName).toBe("t3tools/platform/pulse");
      expect(identity?.owner).toBe("t3tools");
      expect(identity?.name).toBe("pulse");
    }).pipe(Effect.provide(RepositoryIdentityResolverLive)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "pulse-repository-identity-late-remote-test-",
        });
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/t3tools/pulse");
        expect(refreshedIdentity?.name).toBe("pulse");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pulse-repository-identity-remote-change-test-",
      });
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/t3tools/pulse");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:T3Tools/pulse-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/t3tools/pulse");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/t3tools/pulse-next");
      expect(refreshedIdentity?.displayName).toBe("t3tools/pulse-next");
      expect(refreshedIdentity?.name).toBe("pulse-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
