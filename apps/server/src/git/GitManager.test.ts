...
import { GitCommandError, TextGenerationError } from "@pulse/contracts";
...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("pulse-git-remote-");
...
const GitManagerTestLayer = GitVcsDriver.layer.pipe(
  Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "pulse-git-manager-test-" })),
  Layer.provideMerge(VcsProcess.layer),
  Layer.provideMerge(NodeServices.layer),
);
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  it.effect("status includes PR metadata when branch already has an open PR", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("pulse-git-manager-");
  ...
  it.effect("status returns an explicit non-repo result for non-git directories", () =>
    Effect.gen(function* () {
      const cwd = yield* makeTempDir("pulse-git-manager-non-repo-");
      const { manager } = yield* makeManager();
  ...
  it.effect("status returns an explicit non-repo result for deleted directories", () =>
    Effect.gen(function* () {
      const rootDir = yield* makeTempDir("pulse-git-manager-missing-dir-");
              ...
              JSON.stringify([
                {
                  ...
                  url: "https://github.com/sats-labs/pulse/pull/1661",
                  baseRefName: "main",
                  headRefName: "main",
                  state: "OPEN",
                  ...
                  headRepository: {
                    nameWithOwner: "lnieuwenhuis/pulse",
                  },
                  headRepositoryOwner: {
                    login: "lnieuwenhuis",
                  },
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "@sats-lab/pulse/pr-488/statemachine"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.refName).toBe("@sats-lab/pulse/pr-488/statemachine");
        expect(status.pr).toEqual({
          number: 488,
          title: "Rebase this PR on latest main",
          url: "https://github.com/pingdotgg/codething-mvp/pull/488",
              ...
              "effect-atom": JSON.stringify([
                {
                  number: 1618,
                  title: "Correct PR",
                  url: "https://github.com/sats-labs/pulse/pull/1618",
                  baseRefName: "main",
              ...
              "upstream/effect-atom": JSON.stringify([
                {
                  number: 1518,
                  title: "Wrong PR",
                  url: "https://github.com/sats-labs/pulse/pull/1518",
                  baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["remote", "add", "fork-seed", forkDir]);
        yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
        yield* runGit(repoDir, ["checkout", "-b", "@sats-lab/pulse/pr-142/statemachine"]);
        yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
        yield* configureVisibleRemoteUrlWithLocalRewrite(
          repoDir,
          "fork-seed",
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            prListByHeadSelector: {
              // @effect-diagnostics-next-line preferSchemaOverJson:off
              "@sats-lab/pulse/pr-142/statemachine": JSON.stringify([]),
              // @effect-diagnostics-next-line preferSchemaOverJson:off
  ...
  it.effect("creates cross-repo PRs with the fork owner selector and default base branch", () =>
    Effect.gen(function* () {
      ...
      yield* runGit(repoDir, ["commit", "-m", "Feature commit"]);
      yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
      yield* runGit(repoDir, ["checkout", "-b", "@sats-lab/pulse/pr-91/statemachine"]);
      yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
      yield* configureVisibleRemoteUrlWithLocalRewrite(
        repoDir,
        "fork-seed",
      ...
      const { manager } = yield* makeManager({
        ghScenario: {
          pullRequest: {
            ...
            url: "https://github.com/sats-labs/pulse/pull/642",
            baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("@sats-lab/pulse/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
        ...
        ).toBe("@sats-lab/pulse/pr-91/main");
      }),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("@sats-lab/pulse/pr-92/main");
