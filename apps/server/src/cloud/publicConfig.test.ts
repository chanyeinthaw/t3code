...
it.effect("prefers a runtime relay URL override over the statically injected value", () =>
  Effect.gen(function* () {
    const relayUrl = yield* makeRelayUrlConfig("https://embedded.example.test").pipe(
      provideEnv({ PULSE_RELAY_URL: "https://runtime.example.test///" }),
    );
...
it.effect("rejects an insecure runtime relay URL override", () =>
  makeRelayUrlConfig("https://embedded.example.test").pipe(
    provideEnv({ PULSE_RELAY_URL: "http://runtime.example.test" }),
    Effect.flip,
  ),
);
...
it.effect("prefers runtime Clerk OAuth config overrides over statically injected values", () =>
  Effect.gen(function* () {
    const config = yield* makeCloudCliOAuthConfig({
      ...
      provideEnv({
        PULSE_CLERK_PUBLISHABLE_KEY: "pk_test_cnVudGltZS5leGFtcGxlLnRlc3Qk",
        PULSE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_runtime",
      }),
    );
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
  ...
  assert.deepEqual(
    resolveRelayClientTracingConfig(
      {
        PULSE_RELAY_CLIENT_OTLP_TRACES_URL: "https://runtime.example.test/v1/traces",
        PULSE_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        PULSE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ),
    {
  ...
  assert.equal(
    resolveRelayClientTracingConfig(
      {
        PULSE_RELAY_CLIENT_OTLP_TRACES_URL: "http://insecure.example.test/v1/traces",
        PULSE_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        PULSE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
