...
const make = Effect.gen(function* () {
  ...
  const clear = secrets
    ...
    .pipe(wrapError("Could not remove the stored Pulse Connect CLI credential."));
...
<html>
  <body style="font-family: sans-serif; text-align: center; margin-top: 50px;">
    <h1>Pulse Connect authorization complete</h1>
...
`;
    ...
    const code = yield* Deferred.await(callback).pipe(
      ...
      Effect.catchTag("TimeoutError", () =>
        Effect.fail(
          new CloudCliTokenManagerError({
            message: "Timed out waiting for Pulse Connect authorization.",
          }),
        ),
      ),
    );
  ...
  const getExisting = semaphore.withPermits(1)(
    getExistingNoLock().pipe(wrapError("Could not refresh the Pulse Connect CLI credential.")),
  );
  const hasCredential = semaphore.withPermits(1)(
    read().pipe(
      Effect.map(Option.isSome),
      wrapError("Could not read the stored Pulse Connect CLI credential."),
    ),
  );
  const get = semaphore.withPermits(1)(
    ...
    }).pipe(wrapError("Could not authorize the Pulse Connect CLI.")),
  );
