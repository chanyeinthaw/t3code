import { clerkFrontendApiUrlFromPublishableKey } from "@pulse/shared/relayAuth";
...
declare const __PULSE_BUILD_RELAY_URL__: string | undefined;
declare const __PULSE_BUILD_CLERK_PUBLISHABLE_KEY__: string | undefined;
declare const __PULSE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: string | undefined;
declare const __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: string | undefined;
declare const __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: string | undefined;
declare const __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: string | undefined;
...
export const buildTimeRelayUrl =
  typeof __PULSE_BUILD_RELAY_URL__ === "undefined"
    ? ""
    : (normalizeSecureRelayUrl(__PULSE_BUILD_RELAY_URL__) ?? "");
export const buildTimeClerkPublishableKey = readBuildTimeValue(
  typeof __PULSE_BUILD_CLERK_PUBLISHABLE_KEY__ === "undefined"
    ? undefined
    : __PULSE_BUILD_CLERK_PUBLISHABLE_KEY__,
);
export const buildTimeClerkCliOAuthClientId = readBuildTimeValue(
  typeof __PULSE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__ === "undefined"
    ? undefined
    : __PULSE_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__,
);
export const buildTimeRelayClientTracing = {
  tracesUrl: readBuildTimeValue(
    typeof __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__ === "undefined"
      ? undefined
      : __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__,
  ),
  tracesDataset: readBuildTimeValue(
    typeof __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__ === "undefined"
      ? undefined
      : __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__,
  ),
  tracesToken: readBuildTimeValue(
    typeof __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__ === "undefined"
      ? undefined
      : __PULSE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__,
  ),
} as const;
...
export function resolveRelayClientTracingConfig(
  ...
  const tracesUrl = env.PULSE_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
  const tracesDataset =
    env.PULSE_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() || fallback.tracesDataset;
  const tracesToken = env.PULSE_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
  const runtimeConfig = Config.nonEmptyString("PULSE_RELAY_URL");
...
export function makeCloudCliOAuthConfig({
  ...
  return Config.all({
    clerkPublishableKey: makePublicValueConfig(
      "PULSE_CLERK_PUBLISHABLE_KEY",
      clerkPublishableKeyFallback,
    ),
    clientId: makePublicValueConfig(
      "PULSE_CLERK_CLI_OAUTH_CLIENT_ID",
...
export const hasCloudPublicConfig = Boolean(
  (normalizeSecureRelayUrl(process.env.PULSE_RELAY_URL ?? "") ?? buildTimeRelayUrl) &&
  (process.env.PULSE_CLERK_PUBLISHABLE_KEY?.trim() || buildTimeClerkPublishableKey) &&
  (process.env.PULSE_CLERK_CLI_OAUTH_CLIENT_ID?.trim() || buildTimeClerkCliOAuthClientId),
);
