...
import { sanitizeBranchFragment } from "@pulse/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@pulse/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("PULSE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("PULSE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("PULSE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("PULSE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
export class BitbucketApi extends Context.Service<BitbucketApi, BitbucketApiShape>()(
  "@sats-lab/pulse/sourceControl/BitbucketApi",
) {}
...
function checkoutBranchName(input: {
  ...
  return `pulse/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
