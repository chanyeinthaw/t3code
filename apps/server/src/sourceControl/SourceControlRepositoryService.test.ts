...
import { GitCommandError, type SourceControlProviderError } from "@pulse/contracts";
...
const CLONE_URLS = {
  ...
  url: "https://github.com/octocat/pulse",
  sshUrl: "git@github.com:octocat/pulse.git",
};
...
function makeLayer(input: {
  ...
  return SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "pulse-source-control-repos-" })),
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/pulse" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "pulse-source-control-clone-parent-",
    });
    const destinationPath = `${parent}/pulse`;
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "pulse"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/pulse", visibility: "private" },
    ]);
