...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @pulse/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --filter '@pulse/contracts' --filter '@pulse/client-runtime' --filter '@pulse/scripts' --filter '@pulse/web'",
  routes: [
    {
      src: "/__pulse/channel",
