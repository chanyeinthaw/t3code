...
type EnvOption = {
  value: string;
  label: string;
  kind: "local" | "local-worktree" | "worktree";
};
type WorktreeOption = {
  value: string;
  label: string;
...
export const BranchToolbarEnvModeSelector = memo(
  function BranchToolbarEnvModeSelector({
    ...
    const isInitialLoadPending =
      worktreeRefState.isPending && worktreeRefState.data === null;
    ...
    const allWorktreeOptions: WorktreeOption[] = useMemo(() => {
      return allRefs
        .filter(
          (ref) => ref.worktreePath && ref.worktreePath !== activeWorktreePath,
        )
        .map((ref) => {
    ...
    const handleOpenChange = useCallback(
      (open: boolean) => {
        ...
        if (cwd) {
          void vcsRefManager
            .load(worktreeRefTarget, undefined, {
              limit: 100,
              preserveLoadedRefs: true,
            })
            .catch(() => undefined);
        }
      },
    ...
    const handleSelect = useCallback(
      (value: string) => {
        ...
        const worktreeMatch = allWorktreeOptions.find(
          (opt) => opt.value === value,
        );
