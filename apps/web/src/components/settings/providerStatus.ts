import type { ServerProvider, ServerProviderVersionAdvisory } from "@pulse/contracts";
...
export function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in Pulse.",
    };
  }
