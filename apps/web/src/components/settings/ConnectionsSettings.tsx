...
} from "@pulse/client-runtime/connection";
import { findErrorTraceId } from "@pulse/client-runtime/errors";
import {
...
function CloudLinkSwitch({
  ...
  const control = (
    <Switch
      aria-label="Enable Pulse Connect"
      checked={checked}
      disabled={disabled}
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update Pulse Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[pulse-connect] Could not update Pulse Connect", { message, traceId, cause });
  ...
  const updateLink = async (enabled: boolean) => {
    ...
    if (enabled && !tokenResult.value) {
      reportUpdateFailure(new Error("Sign in to Pulse Connect before linking this environment."));
      setIsUpdating(false);
      return;
    }
    ...
    toastManager.add({
      type: "success",
      title: enabled ? "Pulse Connect linked" : "Pulse Connect unlinked",
      description: enabled
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `${environment.label} is available through Pulse Connect.`,
      });
      return;
    }
    ...
    const traceId = findErrorTraceId(cause);
    console.error("[pulse-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. Pulse restarts when you change this."
            className="bg-muted/20 pl-7 sm:pl-8"
            control={
              <Switch
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Pulse will restart to expose this environment over the network."
                    : "Pulse will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Pulse will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Pulse until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Pulse will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Pulse will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Pulse will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Pulse will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Pulse will restart the local backend with Tailscale Serve enabled and ask
