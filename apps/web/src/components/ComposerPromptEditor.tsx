    ...
    >
      <img
        ...
        src={getVscodeIconUrlForEntry(
          props.path,
          inferEntryKindFromPath(props.path),
          theme,
        )}
      />
...
class ComposerMentionNode extends DecoratorNode<React.ReactElement> {
  ...
  static override importJSON(
    ...
    return $createComposerMentionNode(serializedNode.path).updateFromJSON(
      serializedNode,
    );
  }
      ...
      <div className="relative">
        <PlainTextPlugin
          contentEditable={
            <ContentEditable
              className={cn(
                "block max-h-50 min-h-10.5 w-full overflow-y-auto whitespace-pre-wrap wrap-break-word bg-transparent text-[16px] leading-relaxed text-foreground focus:outline-none sm:text-[14px]",
                className,
              )}
          ...
          placeholder={
            terminalContexts.length > 0 ? null : (
              <div className="pointer-events-none absolute inset-0 text-[14px] leading-relaxed text-muted-foreground/35 sm:text-[14px]">
                {placeholder}
              </div>
            )
          }
