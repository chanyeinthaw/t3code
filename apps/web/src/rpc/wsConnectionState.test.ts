...
describe("wsConnectionState", () => {
  ...
  it("schedules the next retry after a failed websocket attempt", () => {
    ...
    recordWsConnectionErrored("Unable to connect to the Pulse server WebSocket.");
  ...
  it("adds a version mismatch hint to websocket errors when metadata includes one", () => {
    ...
    expect(getWsConnectionStatus()).toMatchObject({
      lastError:
        "Unable to connect to the Pulse server WebSocket. Hint: Version mismatch. Try syncing the client and server.",
    });
  });
