...
describe("hostedPairing", () => {
  ...
  it("reads hosted pairing host and query token parameters", () => {
    const url = new URL("https://app.pulse.codes/pair?host=100.64.1.2:3773&token=ABCD1234");
  ...
  it("prefers hash tokens so generated hosted links do not put credentials in search params", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://preview.pulse.codes");
    ...
    expect(url.origin).toBe("https://preview.pulse.codes");
  ...
  it("builds hosted channel selection URLs through the configured router origin", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://app.pulse.codes");
    ...
    expect(url.origin).toBe("https://app.pulse.codes");
    expect(url.pathname).toBe("/__pulse/channel");
  ...
  it("ignores incomplete hosted pairing requests", () => {
    expect(
      hasHostedPairingRequest(new URL("https://app.pulse.codes/pair?host=backend.example.com")),
    ).toBe(false);
    expect(hasHostedPairingRequest(new URL("https://app.pulse.codes/pair?token=ABCD1234"))).toBe(
      false,
    );
  });
  ...
  it("detects the hosted static app only when no backend URL is configured", () => {
    ...
    expect(isHostedStaticApp(new URL("https://preview.pulse.codes/"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://preview.pulse.codes/pair"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://backend.example.com/"))).toBe(false);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://preview.pulse.codes/"))).toBe(false);
  });
  ...
  it("detects hosted channel aliases as static apps", () => {
    ...
    expect(isHostedStaticApp(new URL("https://nightly.app.pulse.codes/"))).toBe(true);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://nightly.app.pulse.codes/"))).toBe(false);
  });
});
