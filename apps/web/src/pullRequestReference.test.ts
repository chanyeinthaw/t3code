...
describe("parsePullRequestReference", () => {
  it("accepts GitHub pull request URLs", () => {
    expect(parsePullRequestReference("https://github.com/chanyeinthaw/pulse/pull/42")).toBe(
      "https://github.com/chanyeinthaw/pulse/pull/42",
    );
  });

  it("accepts Azure DevOps pull request URLs", () => {
    expect(
      parsePullRequestReference("https://dev.azure.com/acme/project/_git/pulse/pullrequest/42"),
    ).toBe("https://dev.azure.com/acme/project/_git/pulse/pullrequest/42");
  });
  ...
  it("accepts legacy Azure DevOps pull request URLs", () => {
    expect(
      parsePullRequestReference("https://acme.visualstudio.com/project/_git/pulse/pullrequest/42"),
    ).toBe("https://acme.visualstudio.com/project/_git/pulse/pullrequest/42");
  });
  ...
  it("accepts gh pr checkout commands with GitHub pull request URLs", () => {
    expect(
      parsePullRequestReference("gh pr checkout https://github.com/chanyeinthaw/pulse/pull/42"),
    ).toBe("https://github.com/chanyeinthaw/pulse/pull/42");
  });
