...
import {
  ...
  setupProjectScript,
} from "@pulse/shared/projectScripts";
...
describe("projectScripts helpers", () => {
  ...
  it("allows overriding runtime env values", () => {
    const env = projectScriptRuntimeEnv({
      project: { cwd: "/repo" },
      extraEnv: {
        PULSE_PROJECT_ROOT: "/custom-root",
        CUSTOM_FLAG: "1",
      },
    });

    expect(env.PULSE_PROJECT_ROOT).toBe("/custom-root");
    expect(env.CUSTOM_FLAG).toBe("1");
    expect(env.PULSE_WORKTREE_PATH).toBeUndefined();
  });
