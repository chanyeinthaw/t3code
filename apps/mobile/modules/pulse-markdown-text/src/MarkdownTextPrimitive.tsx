...
import PulseMarkdownTextRunNativeComponent from "./PulseMarkdownTextRunNativeComponent";
import PulseMarkdownTextNativeComponent from "./PulseMarkdownTextNativeComponent";
...
function MarkdownTextPrimitiveChild({ style, children, ...rest }: MarkdownTextPrimitiveProps) {
  ...
  const nativeChildren = React.Children.toArray(children).map((child) => {
    ...
    return (
      ...
      <PulseMarkdownTextRunNativeComponent
  ...
  if (!isAncestor) {
    return (
      <TextAncestorContext.Provider value={contextValue}>
        <PulseMarkdownTextNativeComponent
          {...textDefaults}
          {...rest}
