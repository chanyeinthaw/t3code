...
#import "PulseMarkdownTextShadowNode.h"
#import "PulseMarkdownTextComponentDescriptor.h"
...
static void PulseMarkdownTextApplyParagraphStyles(
...
static void PulseMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
...
static NSArray<NSDictionary<NSString *, id> *> *PulseMarkdownTextExtractChipBackgrounds(
    NSMutableAttributedString *attributedString,
    const std::vector<PulseMarkdownTextChipRange> &chipRanges)
{
...
@interface PulseMarkdownTextBackingView : UITextView
@property(nonatomic, copy) NSArray<NSDictionary<NSString *, id> *> *chipBackgrounds;
@end

@implementation PulseMarkdownTextBackingView
...
@protocol PulseMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface PulseMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<PulseMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<PulseMarkdownOutsideTapTarget>)target;

@end

static const void *PulseMarkdownOutsideTapCoordinatorKey =
    &PulseMarkdownOutsideTapCoordinatorKey;

@implementation PulseMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<PulseMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<PulseMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, PulseMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<PulseMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static PulseMarkdownOutsideTapCoordinator *
PulseMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  PulseMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, PulseMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[PulseMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        PulseMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface PulseMarkdownText () <RCTPulseMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface PulseMarkdownText () <PulseMarkdownOutsideTapTarget>
@end

@implementation PulseMarkdownText {
  UIView * _view;
  PulseMarkdownTextBackingView * _textView;
  PulseMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<PulseMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const PulseMarkdownTextProps>();
    _props = defaultProps;
    ...
    _textView = [[PulseMarkdownTextBackingView alloc] init];
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    PulseMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, PulseMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [PulseMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<PulseMarkdownTextProps const>(_props);
  ...
  PulseMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  PulseMarkdownTextApplyAttachments(
      convertedAttrString,
      _state->getData().attachmentRanges,
      _attachmentImages);
  _textView.chipBackgrounds = PulseMarkdownTextExtractChipBackgrounds(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::PulseMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::PulseMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<PulseMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<PulseMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<PulseMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == PulseMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == PulseMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == PulseMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == PulseMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const PulseMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (PulseMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[PulseMarkdownTextRun class]]) {
      continue;
    }

    PulseMarkdownTextRun* textChild = (PulseMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::PulseMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::PulseMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> PulseMarkdownTextCls(void)
{
  return PulseMarkdownText.class;
}
