...
fork, with T3's iOS 16 compatibility patch applied. `PulseTerminalView` owns a `libghostty` surface and
...
Android currently implements the same view name (`PulseTerminalSurface`) and event payloads so the
...
```bash
apps/mobile/modules/pulse-terminal/scripts/build-libghostty-ios16.sh
```
