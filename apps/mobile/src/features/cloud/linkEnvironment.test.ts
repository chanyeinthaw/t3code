...
import { RelayMobileClientId } from "@pulse/contracts/relay";
import {
  ...
  it.effect("reuses the status-and-connect token when connecting from the cloud list", () =>
    Effect.gen(function* () {
      const fetchMock = vi.fn((url: string | URL, _init?: RequestInit) => {
        ...
        if (String(url).endsWith("/.well-known/pulse/environment")) {
          return Promise.resolve(
            Response.json({
              environmentId: "env-1",
              label: "Desktop",
      ...
      expect(environmentTokenBody.get("client_label")).toBe("Pulse Mobile");
        ...
        vi.stubGlobal(
          ...
          vi.fn((url: string | URL) =>
            Promise.resolve(
              String(url).endsWith("/v1/client/dpop-token")
                ? Response.json(validDpopAccessTokenResponse("environment:connect"))
                : String(url).endsWith("/.well-known/pulse/environment")
                  ? Response.json({
                      environmentId: "env-other",
                      label: "Other Desktop",
                      platform: { os: "darwin", arch: "arm64" },
