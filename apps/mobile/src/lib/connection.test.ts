...
describe("mobile remote connection records", () => {
  ...
  it("removes hosted pairing credentials while keeping the advertised host", () => {
    expect(
      redactPairingCredential(
        "https://app.pulse.codes/pair?host=https%3A%2F%2Fdesktop.example&token=bootstrap-token&label=Desktop",
      ),
    ).toBe("https://app.pulse.codes/pair?host=https%3A%2F%2Fdesktop.example&label=Desktop");
  });
