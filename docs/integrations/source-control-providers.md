...
Pulse connects directly to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving your editor. Work stays in flow—no more jumping between browser tabs and terminal windows.

## Supported Providers

Pulse works with the platforms your team already uses:
...
- Pulse can suggest titles and descriptions based on your commits
...
1. Install the GitHub CLI on the machine running Pulse:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Pulse and verify GitHub shows as authenticated
...
2. Add these environment variables to the environment running Pulse:
   ```bash
   export PULSE_BITBUCKET_EMAIL="you@example.com"
   export PULSE_BITBUCKET_API_TOKEN="your-token"
   ```
3. Restart Pulse and verify the connection in **Source Control settings**
...
**Git is required** – Pulse uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Pulse (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
