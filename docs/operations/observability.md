...
Pulse has one server-side observability model:
...
```bash
export PULSE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export PULSE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export PULSE_OTLP_SERVICE_NAME=pulse-local
```

Optional:

```bash
export PULSE_TRACE_MIN_LEVEL=Info
export PULSE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `PULSE_OTLP_*`.

macOS app bundle example:

```bash
PULSE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
PULSE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
PULSE_OTLP_SERVICE_NAME=pulse-desktop \
...
```bash
tail -f "$PULSE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$PULSE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$PULSE_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `pulse-local`, `pulse-dev`, or `pulse-desktop`
...
- `PULSE_OTLP_TRACES_URL` was not set
...
- `PULSE_TRACE_FILE`: override trace file path
- `PULSE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `PULSE_TRACE_MAX_FILES`: rotated file count, default `10`
- `PULSE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `PULSE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `PULSE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `PULSE_OTLP_TRACES_URL`: OTLP trace endpoint
- `PULSE_OTLP_METRICS_URL`: OTLP metric endpoint
- `PULSE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `PULSE_OTLP_SERVICE_NAME`: service name, default `pulse-server`
