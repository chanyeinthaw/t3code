...
- `pulse-relay-traces-prod`, an OpenTelemetry trace dataset for Worker requests
- `pulse-relay-otel-ingest-prod`, a dataset-scoped ingest token bound to the Worker
- `pulse-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`pulse-relay-traces-dev-julius` for a personal stage.
