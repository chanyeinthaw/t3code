# Pulse Connect Clerk Setup

Pulse Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `pulse-relay` template with the shared
`pulse-relay` audience.

## Application Keys

Pulse Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
PULSE_CLERK_PUBLISHABLE_KEY=<publishable key>
PULSE_CLERK_JWT_TEMPLATE=<JWT template name>
PULSE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
PULSE_RELAY_URL=https://relay.example.com
```
...
should set `PULSE_CLERK_PUBLISHABLE_KEY`, `PULSE_CLERK_JWT_TEMPLATE`,
`PULSE_CLERK_CLI_OAUTH_CLIENT_ID`, and `PULSE_RELAY_URL` before building. EAS preview and
...
absent, the `pulse connect` CLI command group is omitted. The bundled server still accepts runtime
...
deployment defaults.
`vp run --filter pulse-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
1. Create an OAuth application for the Pulse CLI.
...
5. Set `PULSE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
running Pulse server. The next `pulse serve` or `pulse start` reconciles the relay link and launches the
...
forward that port before running `pulse connect login` or `pulse connect link`:
...
| ------- | -------------------------- |
| Name    | `pulse-relay`              |
| Claims  | `{ "aud": "pulse-relay" }` |

Set `PULSE_CLERK_JWT_TEMPLATE=pulse-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=pulse-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `PULSE_RELAY_URL` still
...
Local desktop development uses `pulse-dev://app`, while packaged builds use `pulse://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `pulse-dev://app`; the production Clerk instance should only need
`pulse://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.pulse.pulse`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.pulse.pulse` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.pulse.pulse`.
...
```dotenv
PULSE_APPLE_TEAM_ID=ABC1234567
PULSE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/pulse.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
PULSE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `PULSE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`PULSE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `PULSE_PORT` set. Rebuild the signed app
...
PULSE_PORT=13773 \
  "/Applications/Pulse (Alpha).app/Contents/MacOS/Pulse (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Pulse (Alpha).app"
codesign -d --entitlements :- "/Applications/Pulse (Alpha).app"
```
...
Approved signed-in users manage Pulse Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the Pulse Connect controls on the Connections page.
