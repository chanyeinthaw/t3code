...
This guide is for people who want to use more than one Claude setup in Pulse.
...
In Pulse Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means Pulse uses your normal home directory.
...
Then add another Claude provider in Pulse:
...
Pulse only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Pulse keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in Pulse Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Pulse stores the value as a server secret and does not
...
Pulse does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in Pulse.
