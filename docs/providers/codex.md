...
This guide is for people who want to use more than one Codex account in Pulse.
...
In Pulse Settings, name it something obvious:
...
In Pulse Settings, add another Codex provider:
...
Pulse shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Pulse treats it
