...
Use this when you want to connect to a Pulse server from another device such as a phone, tablet, or separate desktop app.
...
with the same server-side behavior as `pulse serve --tailscale-serve`, then the server asks Tailscale
...
Use `pulse serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.
...
> For now, use `pulse project ...` on the server machine instead.
> Full GUI support for remote project management is coming soon.

### Option 3: Desktop-Managed SSH Launch

Use this when you want the desktop app to start or reuse Pulse on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote Pulse server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual Pulse server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.t3/ssh-launch/<host-key>/`, starts or reuses a remote Pulse server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Pulse uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Pulse first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Pulse uses:
...
1. `pulse serve` issues a one-time owner pairing token.
...
Hosted pairing does not proxy traffic through Pulse. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `pulse auth` to manage access after the initial pairing flow.
...
Use `pulse auth --help` and the nested subcommand help pages for the full reference.
...
- Use `pulse auth` to revoke credentials or sessions you no longer trust.
