...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("PULSE_PORT_OFFSET"),
  devInstance: optionalStringConfig("PULSE_DEV_INSTANCE"),
});
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      PULSE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.PULSE_PORT = String(serverPort);
      ...
      delete output.PULSE_MODE;
      delete output.PULSE_NO_BROWSER;
      delete output.PULSE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.PULSE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.PULSE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.PULSE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.PULSE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.PULSE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.PULSE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.PULSE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.PULSE_MODE = "web";
      delete output.PULSE_DESKTOP_WS_URL;
    }
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.PULSE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.PULSE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Pulse data (equivalent to PULSE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("PULSE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to PULSE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("PULSE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to PULSE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("PULSE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to PULSE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("PULSE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to PULSE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("PULSE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to PULSE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("PULSE_PORT")),
  ),
